...
class AgentWU(BaseAgent):
    ...
    def completion(self, inp: str) -> Union[str, Generator]:
        ...
        if self._stream == OutputType.Normal:
            args = dict(messages=self.history, functions=[j.schema()['function'] for i, j in self.functions.items()])
            if not args['functions']:
                args.pop("functions")
            resp: ChatCompletionMessage = self.llm.completion(**args)
            ...
            args = dict(history=self.history,messages=self.history, functions=[j.schema()['function'] for i, j in self.functions.items()])
            if not args['functions']:
                args.pop("functions")
            return self.llm.stream(**args)
